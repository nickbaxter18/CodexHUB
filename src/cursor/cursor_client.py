...
class CursorClient:
    ...
    async def make_request(self, endpoint: str, method: str = 'POST', data: Optional[Dict] = None) -> Dict[str, Any]:
        """
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if attempt < self.config.max_retries - 1:
                    ...
                    logger.warning(f"Request attempt {attempt + 1} failed, retrying in {delay}s: {error}")
        
        raise CursorAPIError(f"Request failed after {self.config.max_retries} attempts: {last_error}")
    ...
    async def analyze_security(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        payload = {
            ...
            'checkTypes': request.get('checkTypes', ['injection', 'auth', 'data_leaks', 'dependencies']),
            **request
        }
...
class ArchitectAgentMethods(BaseAgentMethods):
    ...
    async def integrate_omni_laws(self, laws: List[Dict], context: Dict[str, Any]) -> Dict[str, Any]:
        """Integrate omni-laws with system flow orchestration"""
        return await self.client.make_request('/architect/integrate-laws', 'POST', {
            'laws': laws,
            'context': context,
            'integrationStrategy': 'system_flow_orchestration'
        })
    
    async def scaffold_system(self, structure: Dict[str, Any], patterns: List[str]) -> Dict[str, Any]:
        """Scaffold system with omni-domain architecture"""
        return await self.client.make_request('/architect/scaffold', 'POST', {
            'structure': structure,
            'patterns': patterns,
    ...
    async def delegate_structural_exploration(self, requirements: List[str]) -> Dict[str, Any]:
        ...
        return await self.client.make_request('/architect/explore', 'POST', {
...
class FrontendAgentMethods(BaseAgentMethods):
    ...
    async def generate_react_component(self, spec: Dict[str, Any], styling: Dict[str, Any]) -> Dict[str, Any]:
        """Generate React component with Tailwind styling and motion"""
        return await self.client.make_request('/frontend/react-component', 'POST', {
            'spec': spec,
            'styling': styling,
            'framework': 'react',
            'uiLibrary': 'tailwind',
    ...
    async def optimize_ui(self, refinement_type: str, current_code: str) -> Dict[str, Any]:
        ...
        return await self.client.make_request('/frontend/optimize-ui', 'POST', {
            'refinementType': refinement_type,
            'currentCode': current_code,
            'optimizationTarget': 'visual_refinement_and_elevation'
        })
    
    async def accelerate_refactoring(self, code: str, refactoring_goals: List[str]) -> Dict[str, Any]:
        """Accelerate refactoring while preserving visual behavior"""
        return await self.client.make_request('/frontend/refactor', 'POST', {
            'code': code,
            'refactoringGoals': refactoring_goals,
...
class BackendAgentMethods(BaseAgentMethods):
    ...
    async def generate_boilerplate(self, type_name: str, schema: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return await self.client.make_request('/backend/boilerplate', 'POST', {
            'type': type_name,
            'schema': schema,
            'framework': 'fastapi',
    ...
    async def generate_type_definitions(self, schema: Dict[str, Any], language: str = 'python') -> Dict[str, Any]:
        """Generate type definitions with validation"""
        return await self.client.make_request('/backend/types', 'POST', {
            'schema': schema,
            'language': language,
            'includeValidation': True
        })
    
    async def refactor_database_interactions(self, code: str, optimization_goals: List[str]) -> Dict[str, Any]:
        """Refactor database interactions while preserving data integrity"""
        return await self.client.make_request('/backend/db-refactor', 'POST', {
            'code': code,
...
class CICDAgentMethods(BaseAgentMethods):
    ...
    async def suggest_pipeline_steps(self, current_pipeline: Dict[str, Any], requirements: List[str]) -> Dict[str, Any]:
        """Suggest pipeline steps with best practices"""
        return await self.client.make_request('/cicd/pipeline-steps', 'POST', {
            'currentPipeline': current_pipeline,
            'requirements': requirements,
            'bestPractices': True
        })
    
    async def debug_pipeline_failure(self, failure_log: str, context: Dict[str, Any]) -> Dict[str, Any]:
        """Debug pipeline failure with comprehensive analysis"""
        return await self.client.make_request('/cicd/debug', 'POST', {
            'failureLog': failure_log,
            'context': context,
            'analysisDepth': 'comprehensive'
        })
    
    async def generate_missing_tests(self, codebase: Dict[str, Any], coverage_gaps: List[str]) -> Dict[str, Any]:
        """Generate missing tests for coverage gaps"""
        return await self.client.make_request('/cicd/generate-tests', 'POST', {
            'codebase': codebase,
...
class KnowledgeAgentMethods(BaseAgentMethods):
    ...
    async def traverse_brain_blocks(self, blocks: List[Dict[str, Any]], query: str) -> Dict[str, Any]:
        """Traverse brain blocks with intelligent synthesis"""
        return await self.client.make_request('/knowledge/traverse', 'POST', {
            'blocks': blocks,
            'query': query,
            'traversalStrategy': 'intelligent_synthesis'
        })
    
    async def summarize_ndjson_scaffolds(self, scaffolds: List[Dict[str, Any]], context: Dict[str, Any]) -> Dict[str, Any]:
        """Summarize ndjson scaffolds with structured output"""
        return await self.client.make_request('/knowledge/summarize', 'POST', {
            'scaffolds': scaffolds,
            'context': context,
            'outputFormat': 'structured_knowledge'
        })
    
    async def extract_patterns(self, knowledge_blocks: List[Dict[str, Any]], pattern_types: List[str]) -> Dict[str, Any]:
        """Extract patterns with comprehensive depth"""
        return await self.client.make_request('/knowledge/patterns', 'POST', {
...
class QAAgentMethods(BaseAgentMethods):
    ...
    async def run_automated_reviews(self, code: str, review_types: List[str] = None) -> Dict[str, Any]:
        """Run automated reviews with comprehensive depth"""
        if review_types is None:
            review_types = ['accessibility', 'seo', 'ux', 'aesthetics']
            
        return await self.client.make_request('/qa/automated-review', 'POST', {
            'code': code,
            'reviewTypes': review_types,
            'reviewDepth': 'comprehensive'
        })
    
    async def propose_accessibility_fixes(self, code: str, accessibility_issues: List[str]) -> Dict[str, Any]:
        """Propose accessibility fixes with WCAG 2.1 AA compliance"""
        return await self.client.make_request('/qa/accessibility-fixes', 'POST', {
            'code': code,
            'accessibilityIssues': accessibility_issues,
            'complianceLevel': 'WCAG_2_1_AA'
        })
    
    async def suggest_seo_optimizations(self, code: str, seo_context: Dict[str, Any]) -> Dict[str, Any]:
        """Suggest SEO optimizations with comprehensive level"""
        return await self.client.make_request('/qa/seo-optimizations', 'POST', {
            'code': code,
...
class MetaAgentMethods(BaseAgentMethods):
    ...
    async def accelerate_reasoning(self, context: Dict[str, Any], reasoning_type: str) -> Dict[str, Any]:
        """Accelerate reasoning with intelligent synthesis"""
        return await self.client.make_request('/meta/reasoning', 'POST', {
            'context': context,
            'reasoningType': reasoning_type,
            'accelerationStrategy': 'intelligent_synthesis'
        })
    
    async def coordinate_generation(self, task: str, agent_contexts: Dict[str, Any]) -> Dict[str, Any]:
        """Coordinate generation with harmony protocol"""
        return await self.client.make_request('/meta/coordinate-generation', 'POST', {
            'task': task,
            'agentContexts': agent_contexts,
            'coordinationStrategy': 'harmony_protocol'
        })
    
    async def repair_system(self, issue: Dict[str, Any], system_context: Dict[str, Any]) -> Dict[str, Any]:
        """Repair system with comprehensive fix strategy"""
        return await self.client.make_request('/meta/repair', 'POST', {
            'issue': issue,
...
class VisualRefinementCursor:
    ...
    async def compliance_pass(self, code: str, brand_guidelines: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return await self.client.make_request('/visual/compliance', 'POST', {
            'code': code,
            'brandGuidelines': brand_guidelines,
            'passType': 'compliance_check'
        })
    
    async def missed_opportunities_audit(self, code: str, opportunity_types: List[str] = None) -> Dict[str, Any]:
        """Run missed opportunities audit"""
        if opportunity_types is None:
            opportunity_types = ['color_cognition', 'typography', 'layout', 'motion']
            
        return await self.client.make_request('/visual/opportunities', 'POST', {
            'code': code,
    ...
    async def refinement_pass(self, code: str, refinement_goals: List[str]) -> Dict[str, Any]:
        ...
        return await self.client.make_request('/visual/refinement', 'POST', {
            'code': code,
            'refinementGoals': refinement_goals,
            'refinementType': 'visual_elevation'
        })
    
    async def elevation_pass(self, code: str, elevation_level: str = 'luxury_polish') -> Dict[str, Any]:
        """Run elevation pass with luxury polish and cinematic effects"""
        return await self.client.make_request('/visual/elevation', 'POST', {
            'code': code,
