...
async def _execute_with_cursor_agent(func: Callable, agent_type: str, *args, **kwargs):
    ...
    try:
        ...
        # Log usage
        _global_enforcement.log_cursor_usage(agent_type, "execute_function", _get_file_path_from_context(), True)
        
        return result
        
    except Exception as e:
        _global_enforcement.log_cursor_usage(agent_type, "execute_function", _get_file_path_from_context(), False)
...
def validate_cursor_compliance() -> bool:
    ...
    if stats['success_rate'] < 0.8:
        raise CursorEnforcementError(f"Low Cursor integration success rate: {stats['success_rate']:.2%}")
