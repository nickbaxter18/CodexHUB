...
Hash: {brain_block.hash}
    ...
    def _apply_filters(self, answers: List[Dict[str, Any]], query: BrainBlockQuery) -> List[Dict[str, Any]]:
        ...
        if query.section_filter:
            filtered = [a for a in filtered if query.section_filter.lower() in a.get("snippet", "").lower()]
        
        # Filter by tags
        if query.tag_filter:
            filtered = [a for a in filtered if any(tag in a.get("tags", []) for tag in query.tag_filter)]
    ...
    def _is_in_date_range(self, answer: Dict[str, Any], start_date: datetime, end_date: datetime) -> bool:
    ...
    async def get_brain_block_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "content": {
                ...
                "average_length": total_content_length / len(self.brain_blocks) if self.brain_blocks else 0
            },
...
def get_brain_blocks_integration() -> BrainBlocksIntegration:
    ...
    if _global_brain_blocks is None:
        ...
        rules = QAEngine.QARules(version="1.0", agents={}, macros={})
