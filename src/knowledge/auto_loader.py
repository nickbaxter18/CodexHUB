...
class KnowledgeAutoLoader:
    ...
    def _setup_default_sources(self) -> None:
        ...
        default_sources = [
            ...
            KnowledgeSource(
                name="brain_docs",
                path=Path("Brain docs cleansed .ndjson"),
                priority=1
            ),
            # Bundle cleansed NDJSON
            KnowledgeSource(
                name="bundle_docs",
                path=Path("Bundle cleansed .ndjson"),
                priority=2
            ),
    ...
    async def _check_source_changes(self, source: KnowledgeSource) -> None:
        ...
        try:
            if source.path.is_file():
                ...
                if source.last_loaded is None or source.path.stat().st_mtime > source.last_loaded.timestamp():
                ...
                for ndjson_file in source.path.rglob("*.ndjson"):
                    if (source.last_loaded is None or 
                        ndjson_file.stat().st_mtime > source.last_loaded.timestamp()):
